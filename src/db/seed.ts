  ...
  try {
    ...
    const barbershopsToInsert = insertedUsers
      ...
      .map((owner, index) => {
        ...
        return {
          name: nome,
          imageUrl: `https://loremflickr.com/800/600/barbershop,interior?lock=${index}`,
          ownerId: owner.id,
          slug,
    ...
    for (const shop of insertedBarbershops) {
      const servicesForShop = servicosTemplates.map((s) => ({
        name: s.name,
        imageUrl: `https://loremflickr.com/800/600/barber,${s.img}?lock=${Math.floor(Math.random() * 1000)}`,
